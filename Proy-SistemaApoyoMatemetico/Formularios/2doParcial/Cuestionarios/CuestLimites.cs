...
{
    public partial class CuestLimites : Form
    {
        int i, puntos = 0;
        string Opcorrecto;
        ...
        public CuestLimites(int idbusqueda) : this()
        {
            this.idbusqueda = idbusqueda;

        }

        private void CuestLimites_Load(object sender, EventArgs e)
        {
            ...
            i = Convert.ToInt32(rc.scalerReturn("select  min (IDPreg) from Preguntas where IDEx_Fk=5"));
            TxtPregunta.Text = rc.scalerReturn("select Titulo  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
            Rb1.Text = rc.scalerReturn("select opA  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
            Rb2.Text = rc.scalerReturn("select opB  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
            Rb3.Text = rc.scalerReturn("select opC  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
            Rb4.Text = rc.scalerReturn("select opD  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
            Opcorrecto = rc.scalerReturn("select opCorrecto  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=5");
        }
        ...
        private void Siguiente_Click(object sender, EventArgs e)
        {
            ...
            s = rc.scalerReturn("select min(IDPreg) from Preguntas where IDPreg > '" + i + "' and IDEx_Fk = 5");
            if (s.Equals(""))
            {
                ...
                MS.LblNotaT6.Text = puntos.ToString();

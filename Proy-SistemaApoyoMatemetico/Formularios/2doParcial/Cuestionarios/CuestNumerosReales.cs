...
{
    public partial class CuestNumerosReales : Form
    {
        int i, puntos = 0;
        string Opcorrecto;
        ...
        public CuestNumerosReales()
        {
            InitializeComponent();
        }
        public CuestNumerosReales(int idbusqueda) : this()
        {
            this.idbusqueda = idbusqueda;

        }
        private void CuestHiperbola_Load(object sender, EventArgs e)
        {
            ...
            i = Convert.ToInt32(rc.scalerReturn("select  min (IDPreg) from Preguntas where IDEx_Fk=3"));
            TxtPregunta.Text = rc.scalerReturn("select Titulo  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
            Rb1.Text = rc.scalerReturn("select opA  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
            Rb2.Text = rc.scalerReturn("select opB  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
            Rb3.Text = rc.scalerReturn("select opC  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
            Rb4.Text = rc.scalerReturn("select opD  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
            Opcorrecto = rc.scalerReturn("select opCorrecto  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=3");
        }
        ...
        private void Siguiente_Click(object sender, EventArgs e)
        {
            ...
            s = rc.scalerReturn("select min(IDPreg) from Preguntas where IDPreg > '" + i + "' and IDEx_Fk = 3");
            if (s.Equals(""))
            {
                Nt.Tema1 = Convert.ToDecimal(LblNotaT1.Text);
                Nt.Tema2 = Convert.ToDecimal(LblNotaT2.Text);
                Nt.Tema3 = (decimal)puntos;
                ...
                MS.LblNotaT3.Text = puntos.ToString();

...
{
    public partial class CuestLogForm : Form
    {
        int i, puntos = 0;
        string Opcorrecto;
        ...
        public CuestLogForm(int idbusqueda) : this()
        {
            this.idbusqueda = idbusqueda;

        }

        private void CuestLogForm_Load(object sender, EventArgs e)
        {
            ...
            i = Convert.ToInt32(rc.scalerReturn("select  min (IDPreg) from Preguntas where IDEx_Fk=7"));
            TxtPregunta.Text = rc.scalerReturn("select Titulo  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
            Rb1.Text = rc.scalerReturn("select opA  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
            Rb2.Text = rc.scalerReturn("select opB  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
            Rb3.Text = rc.scalerReturn("select opC  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
            Rb4.Text = rc.scalerReturn("select opD  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
            Opcorrecto = rc.scalerReturn("select opCorrecto  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=7");
        }
        ...
        private void Siguiente_Click(object sender, EventArgs e)
        {
            ...
            s = rc.scalerReturn("select min(IDPreg) from Preguntas where IDPreg > '" + i + "' and IDEx_Fk = 7");
            if (s.Equals(""))
            {
                ...
                MS.LblNotaT4.Text = puntos.ToString();

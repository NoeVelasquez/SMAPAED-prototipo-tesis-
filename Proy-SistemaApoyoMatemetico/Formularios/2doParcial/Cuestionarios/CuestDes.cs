...
{
    public partial class CuestDes : Form
    {
        int i, puntos = 0;
        string Opcorrecto;
        ...
        public CuestDes(int idbusqueda) : this()
        {
            this.idbusqueda = idbusqueda;

        }

        private void CuestDes_Load(object sender, EventArgs e)
        {
            ...
            i = Convert.ToInt32(rc.scalerReturn("select  min (IDPreg) from Preguntas where IDEx_Fk=4"));
            TxtPregunta.Text = rc.scalerReturn("select Titulo  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
            Rb1.Text = rc.scalerReturn("select opA  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
            Rb2.Text = rc.scalerReturn("select opB  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
            Rb3.Text = rc.scalerReturn("select opC  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
            Rb4.Text = rc.scalerReturn("select opD  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
            Opcorrecto = rc.scalerReturn("select opCorrecto  from Preguntas where IDPreg = '" + i + "'and IDEx_Fk=4");
        }
        ...
        private void Siguiente_Click(object sender, EventArgs e)
        {
            ...
            s = rc.scalerReturn("select min(IDPreg) from Preguntas where IDPreg > '" + i + "' and IDEx_Fk = 4");
            if (s.Equals(""))
            {
                ...
                MS.LblNotaT5.Text = puntos.ToString();
